      ...
      >
        <nav className="flex items-center justify-between travigo-container">
          <NavLink to={`/`} className="flex items-center">
            <img src={logo} alt="logo/img" className="w-20 h-16 object-fill" />
            <h1 className="font-mono ps-5 text-2xl lg:text-lg">Trave-Lio</h1>
          </NavLink>
          <ul className="flex items-center lg:hidden gap-7">
            {navlinks?.map((val, i) => (
                <li key={i}>
                  <NavLink to={`/${val.link}`} className="text-lg text-slate-900">
                    {val.link}
                  </NavLink>
                </li>
            ))}
